...
bool Mesh::intersectFace(const Ray& ray, Hit& hit, int faceId) const
{
    ...
    FaceIndex face = mFaces[faceId];

    Vertex v0 = mVertices[face(0)];
    Vertex v1 = mVertices[face(1)];
    Vertex v2 = mVertices[face(2)];

    float distance = v0.normal.dot(v0.position);
    float dotNormalRayDir = v0.normal.dot(ray.direction);

    if(dotNormalRayDir > -0.00001f && dotNormalRayDir < 0.00001f)
        return false;

    float t = -(v0.normal.dot(ray.origin) + distance) / dotNormalRayDir;
    ...
    Eigen::Vector3f edge0 = v1.position - v0.position;
    Eigen::Vector3f edge1 = v2.position - v1.position;
    Eigen::Vector3f edge2 = v0.position - v2.position;

    Eigen::Vector3f C0 = P - v0.position;
    Eigen::Vector3f C1 = P - v1.position;
    Eigen::Vector3f C2 = P - v2.position;

    Eigen::Vector3f normal = v0.normal;

    if(normal.dot(edge0.cross(C0)) > 0.f && normal.dot(edge1.cross(C1)) > 0.f && normal.dot(edge2.cross(C2)) > 0.f)
        return true;
...
bool Mesh::intersect(const Ray& ray, Hit& hit) const
{
    ...
    else
    {
        ...
        hit.setT(tMin);
        hit.setNormal(Eigen::Vector3f(0,-1,0));
        ret = true;
